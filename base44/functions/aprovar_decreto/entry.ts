...
// Aprova (ou rejeita) uma ingestão de DECRETO conferida por um humano. Ao
// aprovar, aplica as regras conferidas ao convênio escolhido via a RPC atômica
// aplicar_regras_decreto (COALESCE preserva o que já existe). Nada muda no
// convênio sem passar por aqui. Espelho no Drive é best-effort.
...
Deno.serve(async (req) => {
  ...
  const convenioId: string = body.convenio_id || '';
  ...
  if (ing.tipo_documento !== 'decreto') return Response.json({ error: 'Esta ingestão não é um decreto.' }, { status: 400 });
  ...
  // Rejeição → não altera convênio.
  if (acao === 'rejeitar') {
    await admin.from('ingestoes_documento').update({
      status: 'rejeitado', observacao: justificativa || 'Rejeitada.',
      aprovado_por: caller.user.id, aprovado_em: new Date().toISOString(),
    }).eq('id', ingestaoId);
    await admin.from('auditoria').insert({ entidade: 'ingestoes_documento', registro_id: ingestaoId, acao: 'ingestao_decreto_rejeitar', empresa_id: ing.empresa_id, valor_novo: { justificativa } });
    return Response.json({ id: ingestaoId, status: 'rejeitado' });
  }

  if (acao !== 'aplicar') return Response.json({ error: 'Ação inválida (use aplicar | rejeitar).' }, { status: 400 });
  if (!convenioId) return Response.json({ error: 'Selecione o convênio para aplicar as regras.' }, { status: 400 });

  // Divergência crítica (ex.: confiança baixa) exige justificativa.
  ...
  if (temCritica && !justificativa.trim()) {
    return Response.json({ error: 'Há divergência crítica — justificativa é obrigatória para aplicar.' }, { status: 400 });
  }
  ...
  const { data: convId, error: rpcErr } = await userClient.rpc('aplicar_regras_decreto', {
    p_ingestao: ingestaoId, p_convenio: convenioId, p_dados: dados, p_justificativa: justificativa || null,
  });
  if (rpcErr) return Response.json({ error: rpcErr.message }, { status: 400 });

  // Espelho no Drive — best-effort, NUNCA afeta o resultado.
  ...
  return Response.json({ id: ingestaoId, status: 'aprovado', convenio_id: convId });
});
