...
import { Plus, Send, MessagesSquare, ShieldCheck, AlertTriangle, X, UserPlus, Clock } from 'lucide-react';
...
const ST_COR = {
  ...
  reaberto: 'bg-amber-50 text-amber-700', cancelado: 'bg-muted text-muted-foreground', encerrado: 'bg-muted text-muted-foreground',
};
const PRIOR = { baixa: 'Baixa', media: 'Média', alta: 'Alta', critica: 'Crítica' };
const PR_COR = { baixa: 'text-muted-foreground', media: 'text-muted-foreground', alta: 'text-amber-600', critica: 'text-red-600' };
const CATEGORIA = {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <p className="text-sm text-muted-foreground">Chamados & pendências ancorados a registros — colaboração auditável</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Caixa de entrada */}
        <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden lg:max-h-[72vh] lg:overflow-y-auto">
          {loading ? <div className="p-3 space-y-2">{Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-14 rounded-lg bg-muted animate-pulse" />)}</div>
          : view.length === 0 ? <EmptyState icon={MessagesSquare} title="Nenhum item." />
          : view.map((t) => (
            <button key={t.id} onClick={() => abrir(t)} className={`w-full text-left px-4 py-3 border-b border-border hover:bg-muted/50 ${sel?.id === t.id ? 'bg-primary/5' : ''}`}>
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-medium text-foreground truncate">{t.titulo}</p>
                <StatusBadge className={`text-[10px] shrink-0 ${ST_COR[t.status]}`}>{STATUS[t.status]}</StatusBadge>
              </div>
        ...
        {/* Thread */}
        <div className="lg:col-span-2 bg-card rounded-xl border border-border shadow-sm flex flex-col lg:max-h-[72vh]">
          {!sel ? (
            <div className="flex-1 flex items-center justify-center">
              <EmptyState icon={MessagesSquare} title="Selecione um chamado para ver a conversa." />
            </div>
          ) : (
            <>
              <div className="p-4 border-b border-border">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="font-semibold text-foreground">{sel.titulo}</p>
                    {sel.descricao && <p className="text-xs text-muted-foreground mt-0.5">{sel.descricao}</p>}
                  ...
                  {sel.requer_validacao && (
                    <StatusBadge className="text-[10px] gap-1 shrink-0 bg-orange-50 text-orange-700"><ShieldCheck className="w-3 h-3" /> Requer validação</StatusBadge>
                  )}
                </div>
                <div className="flex flex-wrap items-center gap-2 mt-3">
                  ...
                  {sel.prazo_sla && (
                    <span className={`text-[11px] flex items-center gap-1 ${atrasada(sel) ? 'text-red-600' : 'text-muted-foreground'}`}><Clock className="w-3 h-3" /> {hora(sel.prazo_sla)}</span>
                  )}
                  {podeValidar && (
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[240px]">
                {msgs.length === 0 ? (
                  <EmptyState title="Sem mensagens." description="Inicie a conversa." />
                ) : msgs.map((m) => {
                  if (m.tipo === 'sistema') {
                    return (
                      <div key={m.id} className="flex justify-center">
                        <span className="text-[10px] text-muted-foreground bg-muted border border-border rounded-full px-3 py-1">⚙ {m.corpo} · {hora(m.created_at)}</span>
                      </div>
                    );
                  }
                  ...
                  return (
                    <div key={m.id} className={`flex ${meu ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] rounded-2xl px-3 py-2 ${meu ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground'}`}>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className={`text-[9px] ${meu ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>{hora(m.created_at)}</span>
                ...
                <div className="flex items-end gap-2">
                  <div className="w-40">
                    <Select value="" onValueChange={(v) => setMencoes((s) => (s.includes(v) ? s : [...s, v]))}>
                      <SelectTrigger className="h-9 text-xs"><span className="flex items-center gap-1 text-muted-foreground"><UserPlus className="w-3.5 h-3.5" /> Mencionar</span></SelectTrigger>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={criar} className="space-y-4">
            ...
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider pt-1">Âncora (obrigatória)</p>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
