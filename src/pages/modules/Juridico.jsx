...
function LgpdTab() {
  ...
  if (!isAdmin) return <p className="text-sm text-muted-foreground">As solicitações LGPD são restritas a administradores do grupo.</p>;

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">Solicitações de titular (LGPD) — acesso, correção, exclusão, portabilidade</p>
      ...
      <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
        ...
        : itens.length === 0 ? <EmptyState title="Nenhuma solicitação." />
        : (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border bg-muted/50">
              <th className="text-left px-4 py-3 font-medium text-muted-foreground uppercase text-xs">Titular</th>
              <th className="text-left px-4 py-3 font-medium text-muted-foreground uppercase text-xs">Tipo</th>
              <th className="text-right px-4 py-3 font-medium text-muted-foreground uppercase text-xs hidden md:table-cell">Prazo</th>
            ...
            <tbody>
              {itens.map((s) => (
                <tr key={s.id} className="border-b border-border hover:bg-muted/50">
                  <td className="px-4 py-3 font-medium text-foreground">{s.titular_nome}</td>
                  <td className="px-4 py-3 text-muted-foreground">{TIPO[s.tipo]}</td>
                  <td className="px-4 py-3 text-right text-muted-foreground hidden md:table-cell">{dataBR(s.prazo_limite)}</td>
                  <td className="px-4 py-3"><StatusBadge className={CORES[s.status]}>{STATUS[s.status]}</StatusBadge></td>
                  <td className="px-4 py-3 text-right"><button onClick={() => openEdit(s)} className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded"><Pencil className="w-4 h-4" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
